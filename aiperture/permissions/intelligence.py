...
from aiperture.models.verdict import GlobalSignal
...
class IntelligenceEngine:
    ...
    def report_decision(
        ...
        """Report a decision — delegates to plugin backend if available."""
        from aiperture import plugins

        backend = plugins.get("intelligence_backend")
        if backend is not None:
            backend.report_decision(tool, action, scope, decision_is_allow)
            return
        self._report_decision_default(tool, action, scope, decision_is_allow, epsilon)

    def _report_decision_default(
        self,
        tool: str,
        action: str,
        scope: str,
    ...
    def get_global_signal(
        ...
        backend = plugins.get("intelligence_backend")
        if backend is not None:
            return backend.get_global_signal(tool, action, scope)
